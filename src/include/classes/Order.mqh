...
class Order : public SymbolInfo {
  ...
  bool ShouldCloseOrder() {
    ...
    if (oparams.HasCloseCondition()) {
      ...
      for (int _ci = 0; _ci < _num; _ci++) {
        ...
        if (Order::CheckCondition(_cond, _cond_args)) {
          double _arg = DataParamEntry::ToDouble(_cond_args[0]);
          long _ppips = Get<long>(ORDER_PROP_PROFIT_PIPS);
          PrintFormat("[Order] Close condition hit: ticket=%I64d cond=%d arg=%.2f profit_pips=%ld",
                      (long)Get<long>(ORDER_PROP_TICKET), (int)_cond, _arg, _ppips);
          _result = true;
        }
      }
    }
  ...
  */
  ...
  bool OrderClose(ENUM_ORDER_REASON_CLOSE _reason = ORDER_REASON_CLOSED_UNKNOWN, string _comment = "") {
    ...
    MqlTradeResult _result = {0};
    _request.action = TRADE_ACTION_DEAL;
    string _comment_text = _comment != "" ? _comment : odata.GetReasonCloseText();
    _request.comment = _comment_text;
    PrintFormat("[Order] Closing: ticket=%I64d reason=%d comment=%s profit_pips=%ld",
                (long)odata.Get<ulong>(ORDER_PROP_TICKET), (int)_reason, _comment_text,
                (long)Get<long>(ORDER_PROP_PROFIT_PIPS));
